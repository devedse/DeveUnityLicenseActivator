...
$7zFilePath = Join-Path $outputDir "DeveUnityLicenseActivator.7z"
$zipFilePath = Join-Path $outputDir "DeveUnityLicenseActivator.zip"

DeleteFileIfExists $7zFilePath
DeleteFileIfExists $zipFilePath
DeleteFolderIfExists $outputDir

$buildPath = Join-Path $solutionRoot "DeveUnityLicenseActivator\bin\Release\netstandard2.1"
